    ...
    function educatorCard(ed) {
      ...
      function renderPanel(kind, reset = false) {
        ...
        if (end < all.length) {
          ...
          // <CHANGE> Add stopPropagation to prevent Load More button click from bubbling to card
...
async def get_all_educators(skip: int = Query(0, ge=0), limit: int = Query(5, ge=1)):
    # <CHANGE> Add deduplication to prevent duplicate educators from appearing on initial load
    seen = set()
    educators = []
