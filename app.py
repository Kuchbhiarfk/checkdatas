...
<body>
  ...
  <script>
    ...
    async function renderSearchGroup(group, reset = false) {
      ...
      const {data, total} = await fetchWithDisable(null, `/${endpoint}=${encodeURIComponent(state.searchQuery)}&skip=${skip}&limit=${PAGE}`);
...
@app.get("/data")
async def get_all_data(skip: int = Query(0, ge=0), limit: int = Query(5, ge=1)):
    total = educators_col.count_documents({})
    cursor = educators_col.find({}).skip(skip).limit(limit)
    data = [map_educator(doc) for doc in cursor]
...
@app.get("/batches")
async def get_all_batches(skip: int = Query(0, ge=0), limit: int = Query(5, ge=1), status: str = Query(None, enum=["live", "completed", None])):
    batches = []
    for educator in educators_col.find({}, {"_id": 0, "batches": 1}):
        for batch in educator.get("batches", []):
            batch_status = get_status(map_batch(batch))
            if not status or batch_status == status:
                batches.append(map_batch(batch))
    if status == "live":
        batches.sort(key=lambda b: datetime.fromisoformat(b["startDate"].replace("Z", "+00:00")), reverse=True)
    elif status == "completed":
        batches.sort(key=lambda b: datetime.fromisoformat(b["endDate"].replace("Z", "+00:00")), reverse=True)
    total = len(batches)
    data = batches[skip: skip + limit]
    return JSONResponse({"data": data, "total": total})

@app.get("/courses")
async def get_all_courses(skip: int = Query(0, ge=0), limit: int = Query(5, ge=1), status: str = Query(None, enum=["live", "completed", None])):
    courses = []
    for educator in educators_col.find({}, {"_id": 0, "courses": 1}):
        for course in educator.get("courses", []):
            course_status = get_status(map_course(course))
            if not status or course_status == status:
                courses.append(map_course(course))
    if status == "live":
        courses.sort(key=lambda c: datetime.fromisoformat(c["startDate"].replace("Z", "+00:00")), reverse=True)
    elif status == "completed":
        courses.sort(key=lambda c: datetime.fromisoformat(c["endDate"].replace("Z", "+00:00")), reverse=True)
    total = len(courses)
    data = courses[skip: skip + limit]
    return JSONResponse({"data": data, "total": total})

@app.get("/search_educator={keyword}")
async def search_educator(keyword: str, skip: int = Query(0, ge=0), limit: int = Query(5, ge=1)):
    query = {
        "$or": [
    ...
    total = educators_col.count_documents(query)
    cursor = educators_col.find(query).skip(skip).limit(limit)
    data = [map_educator(doc) for doc in cursor]
    return JSONResponse({"data": data, "total": total})

@app.get("/search_batch={keyword}")
async def search_batch(keyword: str, skip: int = Query(0, ge=0), limit: int = Query(5, ge=1)):
    batches = []
    query = {
        "$or": [
...
async def search_courses(keyword: str, skip: int = Query(0, ge=0), limit: int = Query(5, ge=1)):
    courses = []
    query = {
        "$or": [
